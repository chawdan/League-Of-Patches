...
public class RuneAggregateBean {
	...
	public RuneAggregateBean(int wins, int qty, int runeId, int rank, int division, int assists, int deaths, int kills,
			int qtyUsed, HashMap<String, Integer> laneQty, HashMap<String, Integer> roleQty, int weekDate, int yearDate) {
		super();
		this.wins = wins;
		this.qty = qty;
		this.runeId = runeId;
	...
	public static RuneAggregateBean[] playerMatchBeanToRuneAggregateBean(PlayerMatchBean bean, RankBean rankBean){
		...
		for(int i = 0; i < items.length; i++){
			...
			beans.add(new RuneAggregateBean(wins,  qty,  itemId.getRuneId(),  rank,  division, assists, deaths,  kills,
					itemId.getRuneRank(),laneQty, roleQty, weekDate, yearDate));
		}
